...
pub async fn get_events(
    ...
    let mut url = Url::parse("https://valorant-api.com/v1/events").unwrap();
...
pub async fn get_event(
    ...
    event: Uuid,
    language: Option<Language>,
) -> reqwest::Result<Event> {
    let mut url = Url::parse(&format!("https://valorant-api.com/v1/events/{}", event)).unwrap();
    if let Some(language) = language {
        url.query_pairs_mut()
            .append_pair("language", &format!("{}", language));
    }
